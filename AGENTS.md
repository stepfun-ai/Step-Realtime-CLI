...
>
> **本仓定位**：`step-code-pi` 是 pi 探索专用副本（建仓基线 `12af693`，remote 已摘），起于
> 验证 pi-tui 能否替代 Ink（M1-M5 已完成，13 项交互对标已落地），现按「同步主仓非 TUI 层、
> TUI 层走自己的路」推进。迁移设计与同步机制归档在内部设计仓。
>
> **与主仓同步（硬约束，先读规范再动手）**：两仓同步规则以产品设计仓
> `两仓关系与差异.md` 的「两版本同步规范（权威）」一节为唯一事实源，本文不重复。
> 三条最易违反的，在此强调：
> 1. **L2 共享层（agent 核心）双向、逐字节一致**：改了 `src/tools`/`provider`/`session`/`config`/`skill`/`agent` 等，
>    必须同步到主仓，分别在两仓 CHANGELOG 登记、在同步档案记账。禁止静默单边改。
> 2. **L1 特性对标双向**：本仓新落地一个用户可见特性，要在同步周期内评估主仓侧「实现 / 明确判不做」并登记。
> 3. **L3 TUI 层只翻译不同步代码**：主仓改 `App.tsx` 的意图在 `PiChat` 重新实现，不照搬。
> 操作细节（路径映射、打 patch 命令、已同步范围）见 `pi-tui实验/20260816-主仓同步机制与已同步范围.md`。
>
> 本文件其余内容从主仓带来，描述的是 Ink 版架构；在本仓做 TUI 相关改动时以 `src/tui-pi/`
> 的实际代码与设计档案为准。
>
