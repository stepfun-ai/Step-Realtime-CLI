...
>
> **与主仓同步**：两仓不共享 git 历史，同步靠逐 commit 打 patch + `src/tui` → `src/chat` 路径映射，
> 碰 `App.tsx` 的部分人工翻译到 `PiChat`。已同步至主仓 `fed85fb`（2026-08-16）。做同步前先读
> 设计仓 `pi-tui实验/20260816-主仓同步机制与已同步范围.md`，别自己重新摸索一遍。
>
