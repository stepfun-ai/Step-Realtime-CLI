...
describe("buildTranscriptItems", () => {
  it("returns welcome item as first entry", () => {
    const items = buildTranscriptItems([], 80, MOCK_THEME, false);
  ...
  it("sets useMarkdown=true for assistant messages", () => {
    ...
    const items = buildTranscriptItems(entries, 80, MOCK_THEME, false);
...
describe("transcript visibility and streaming", () => {
  it("omits hidden entries", () => {
    const hidden = { ...entry("system", "internal"), hidden: true };
    const items = buildTranscriptItems([hidden], 80, MOCK_THEME, false);
    expect(items).toHaveLength(1);
  });

  it("keeps assistant Markdown in streaming mode until completion", () => {
    const streaming = { ...entry("assistant", "```ts"), streaming: true };
    const items = buildTranscriptItems([streaming], 80, MOCK_THEME, false);
    expect(items[1]!.useMarkdown).toBe(true);
    expect(items[1]!.streaming).toBe(true);
  });

  it("keeps tool and reasoning output collapsible", () => {
    const entries = [
      entry("tool", "[completed]\nfirst\nsecond\nthird\nfourth"),
      entry("reasoning", "first\nsecond\nthird"),
    ];
    const items = buildTranscriptItems(entries, 80, MOCK_THEME, false);
    expect(items[1]!.collapsible).toBe(true);
    expect(items[1]!.expanded).toBe(false);
    expect(items[2]!.collapsible).toBe(true);
  });
});
