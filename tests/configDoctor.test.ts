...
describe('runDoctorConfig', () => {
  ...
  it('空文件（合法空表）→ code 0', () => {
    ...
    expect(runDoctorConfig(p).code).toBe(0);
  });

  it('未知顶层键 → code 0 + warn 点名', () => {
  ...
  it('文件不存在 → code 1', () => {
    const res = runDoctorConfig(join(dir, 'nope.toml'));
    expect(res.code).toBe(1);
    expect(res.stderr).toContain('配置文件不存在');
  });

  it('thinking budget 余量不足 → code 1（复用 loadConfig 抛错路径）', () => {
  ...
  it('thinking default_level 未命中档位 → code 1', () => {
