...
describe('resolveProxy', () => {
  ...
  it('非法值抛配置错误（doctor 复用抛错路径 exit 1）', () => {
...
describe('doctor proxy 校验', () => {
  it('合法 proxy 配置 doctor 通过', () => {
    writeHomeConfig('proxy = "http://127.0.0.1:7892"\n');
    const r = runDoctorConfig(join(dir, '.step-code', 'config.toml'));
    expect(r.code).toBe(0);
  });

  it('非法 proxy 配置 doctor exit 1', () => {
